...
def cost_function(theta, X, y):
    ...
    Hy = - (y.dot(np.log(np.maximum(y_preds, FLT_POSITIVE_MIN))) + (1.0 - y).dot(
        np.log(np.maximum(1.0 - y_preds, FLT_POSITIVE_MIN))))
...
def newton_optimizer(theta_init, X, y, iters=200, converge_change=-1e-60):
    ...
    logging.info(
        '\t\t\tRMS error: {error}'.format(error=np.sqrt(np.mean((logistic_regression(theta, X_i, y_i) - y_i) ** 2))))

    change_loss = old_loss - loss

    return theta, loss_history  # It is the same as linear regression except for h_theta(x) = sigmoid(x)
...
def optimizer(theta_init, X, y, iters=200, converge_change=-1e-60):
    ...
    for i in range(iters):
        ...
        loss_history.append(loss)
        logging.info("iter: {:8d}\n\t\t\tloss: {:6f}".format(i, loss))
        logging.info('\t\t\tRMS error: {error}'.format(
            error=np.sqrt(np.mean((logistic_regression(theta, X_i, y_i) - y_i) ** 2))))
...
if __name__ == '__main__':
    ...
    origin_x_y = np.load('./ex4_dataset.npy')

    origin_x_y = np.hstack((np.ones((origin_x_y.shape[0], 1)), origin_x_y))
    origin_x = origin_x_y[:, :3]
    print origin_x
    origin_y = origin_x_y[:, 3]
    train_size = int(0.8 * origin_x.shape[0])
    # orig_train_x, train_y, orig_test_x, test_y = split_digits_data(digits)
    origin_train_x = origin_x[:train_size, :]
    origin_train_y = origin_y[:train_size]
    origin_test_x = origin_x[train_size:, :]
    origin_test_y = origin_y[train_size:]

    theta_init = np.random.rand(origin_train_x.shape[1])
    # theta_init = np.zeros(orig_train_x.shape[1]) + 0.0001
    print origin_test_x, origin_test_y
    optimal_theta_1 = gradient_descend(origin_train_x, origin_train_y, origin_test_x, origin_test_y, theta_init,
                                       sv_name='./results/gd_logistic.png', iters=1000)
    ...
    prob_y = logistic_regression(optimal_theta_1, origin_test_x, origin_test_y)
    pred_y = np.where(prob_y >= 0.5, 1, 0) == origin_test_y

    logging.info(
        'TP:{} Vs Total:{},  Accuracy: {}'.format(np.sum(pred_y), len(pred_y), np.sum(pred_y) * 1. / len(pred_y)))
